...
//Tag used for the ESP serial console messages
...
/**
	...
	* @param agr data, aside from event data,that is passed to the handler when it is called
...
static void wifi_app_soft_ap_config(void)
{
	wifi_config_t ap_config =
	{
		.ap = {
			...
			.max_connection = MAX_CONNECTION_RETRIES, //WIFI_AP_MAX_CONNECTIONS
	...
	ESP_ERROR_CHECK(esp_netif_set_ip_info(esp_netif_ap, &ap_ip_info)); // Startically configure the network interface
	...
	ESP_ERROR_CHECK(esp_wifi_set_mode(WIFI_MODE_APSTA)); // Setting the mode as Access Point Mode
		...
		if(xQueueReceive(wifi_app_queue_handle, &msg, portMAX_DELAY))
		{
			switch (msg.msgID)
			{
				...
				case WIFI_APP_MSG_CONNECTING_FROM_HTTP_SERVER:
					...
					break;
				case WIFI_APP_STA_CONNECTED_GOT_IP:
					ESP_LOGI(TAG, "WIFI_APP_STA_CONNECTED_GOT_IP");
					break;
				default:
					break;		
			}
...
void wifi_app_start(void)
{
	...
	// Create meassage queue
	wifi_app_queue_handle = xQueueCreate(3, sizeof(wifi_app_queue_message_t));
	//Start the WiFi application task
	xTaskCreatePinnedToCore(&wifi_app_task, "wifi_app_task", WIFI_APP_TASK_STACK_SIZE, NULL, WIFI_APP_TASK_PRIOTITY, NULL, WIFI_APP_TASK_CORE_ID);
}